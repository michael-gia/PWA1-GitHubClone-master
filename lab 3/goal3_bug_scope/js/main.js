 ...
 ***********************/
	...
	var name = james, course = 'PWA1, month = 3; // the string is not closed here, therefore making this comment a string.
	...
	var phrase = 'he's in ' + ' "PWA1' ";// the s in hes is outside of the string and inconsistant quotes are being used
	console.log("phase 4: ", phr@se); //phase is misspelled with an at symbol
	...
	var mood = function(weather, waves) // no open curly brackets to define the container for the function

		if (weather = "Sunny"){ //3 equal signs need to be here
			if (waves === 10){
				Mood = 'PUMPED';
			}else if ((wave < 9) && (waves >= 5)){ //wave should have an s on the end
				Mood = 'mellow';			
			}else{
				Mood = 'bummed';
			};	
			...
			if ((wave <= 10) !! (waves >= 7)){ //wave should have an s on the end of it
				Mood = "JACKED UP";
		...
		return Moood; //mood has an extra o.
	};
    ...
    for (var i=10, j=myNums.length; i < j; i++){
        ...
        continue;
        console.log('What is at index ' + i + ' = ', myNums[i] ); // calling the array myNums[i] does not call anything in the array
    ;
